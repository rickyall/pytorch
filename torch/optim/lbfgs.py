...
class LBFGS(Optimizer):
    ...
    def __init__(self, params, lr=1, max_iter=20, max_eval=None,
            tolerance_fn=1e-5, tolerance_x=1e-9, n_correction=100,
            line_search_fn=None):
        if max_eval is None:
            max_eval = max_iter * 1.25
        defaults = dict(lr=lr, max_iter=max_iter, max_eval=max_eval,
                        tolerance_fn=tolerance_fn, tolerance_x=tolerance_x,
                        n_correction=n_correction, line_search_fn=line_search_fn)
    ...
    def step(self, closure):
        ...
        tolerance_fn = group['tolerance_fn']
        tolerance_x = group['tolerance_x']
        line_search_fn = group['line_search_fn']
        n_correction = group['n_correction']
        ...
        while n_iter < max_iter:
            ...
            if state['n_iter'] == 1:
                ...
                y = torch.add(flat_grad, -1, prev_flat_grad)
                s = d.mul(t)
                ys = y.dot(s)  # y*s
                if ys > 1e-10:
                    # updating memory
                ...
                if 'ro' not in state:
                    state['ro'] = [None] * n_correction
                    state['al'] = [None] * n_correction
                ro = state['ro']
                al = state['al']
            ...
            if flat_grad.abs().sum() <= tolerance_fn:
                break

            if d.mul(t).abs_().sum() <= tolerance_x:
                break

            if abs(loss - prev_loss) < tolerance_x:
                break
